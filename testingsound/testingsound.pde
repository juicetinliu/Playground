  ...
  //if(startgam && shape.size() > 2){
    if(createsounds){
      for(int i = 0; i < partnumber; i++){
        sounds.add(new soundParticle(playerx, playery, radians((360*i)/partnumber), 3,1000));
      }
      createsounds = false;
    }
    ...
    //  soundP.bounce(shape.get(shape.size()-1).x,shape.get(shape.size()-1).y,shape.get(0).x,shape.get(0).y);
      ...
      //text(soundP.bounces,soundP.trail.get(0).x,soundP.trail.get(0).y);
      //soundP.move();
    }
    if(bounceit){
      bounceit = false;
