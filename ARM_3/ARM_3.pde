...
void draw() {
  ...
  drawrover();
  if(a[0] > 115 || a[0] < -115 || a[1] > 130 || a[1] < -130 || a[2] > 130 || a[2] < -130 ){
    if(once == true){
      ...
      strokeWeight(8.0);
      stroke(255, 128); 
      ellipse(fx[0], fy[0], 10, 10);
      line(fx[0], fy[0], fx[1], fy[1]);
      ellipse(fx[1], fy[1], 10, 10);
      line(fx[1], fy[1], fx[2], fy[2]);
      ellipse(fx[2], fy[2], 10, 10);
      line(fx[2], fy[2], fx[3], fy[3]);
      ellipse(fx[3], fy[3], 10, 10);
    }else{
      strokeWeight(8.0);
      stroke(255, 128); 
      ellipse(fx[0], fy[0], 10, 10);
...
void changearms(){
  if(currentarm == 1){
    ...
    if(sq(mouseX - 250) + sq(mouseY - 250) < sq(2 * segLength)){
      ...
      //change
      float square = sqrt(sq(dx2) + sq(dy2));
      if(square > 100){
        square = 100;
      }
      float angseg2 = flip * acos(square / (2 * segLength));
      float changeang = angseg2 - angseg1 + angle1 - angle2;
      print(">>" + sqrt(sq(dx2) + sq(dy2)) + "<<");
      ...
      if(controlend){
        x[3] = x[2] + cos(xangp3 + changeang) * segLength;
        y[3] = y[2] + sin(xangp3 + changeang) * segLength;
        //print(y[3] + " " + y[2] + " : " + x[3] + " " + x[2] + " : " + changeang + " <| ");
        //text(xangp3, 100, 45);
      ...
      //x[1] = x[0]+segLength*cos(xangp1 + changeang);
      //y[1] = y[0]+segLength*sin(xangp1 + changeang);
      //x[2] = x[1]+segLength*cos(xangp2 + changeang);
      //y[2] = y[1]+segLength*sin(xangp1 + changeang);
      if(controlend){
        x[3] = x[2] + cos(xangp3 + changeang) * segLength;
        y[3] = y[2] + sin(xangp3 + changeang) * segLength;
        print(y[3] + " " + y[2] + " : " + x[3] + " " + x[2] + " : " + changeang + " =| ");
